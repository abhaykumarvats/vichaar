    ...
    if (req.query.vishay) {
        ...
        Vichaar.count({ vishay: req.query.vishay }, (err, count) => {
            if (err) {
                ...
                Vichaar.find({ vishay: req.query.vishay }, { _id: 0, __v: 0, bhaasha: 0 }).skip(random).limit(1).exec((err, vichaar) => {
                    if (err) {
                        // Log error, if any
                        console.log(err);
                    } else {
        ...
        Vichaar.count((err, count) => {
            if (err) {
                ...
                Vichaar.findOne({ sankhya: random }, { _id: 0, __v: 0, bhaasha: 0 } , (err, vichaar) => {
                    if (err) {
                        // Log error, if any
                        console.log(err);
                    } else {
